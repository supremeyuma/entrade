<x-layouts.admin>
    ...
    <div class="space-y-3 sm:space-y-6">
            ...
            <div class="relative px-3.5 py-4 sm:px-8 sm:py-8">
                ...
                <div class="relative flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <h1 class="mt-1.5 text-xl font-semibold tracking-tight sm:mt-3 sm:text-4xl {{ $headingClasses }}">Create FAQ</h1>
                        <p class="mt-1 text-xs sm:text-sm {{ $mutedTextClasses }}">Add a new knowledge base entry with compact mobile-friendly controls.</p>
                    </div>
        ...
        <section data-aos="fade-up" data-aos-delay="120" class="rounded-[20px] sm:rounded-[28px] border p-3.5 sm:p-6 shadow-sm transition duration-300 ease-out hover:-translate-y-1 hover:shadow-xl {{ $surfaceClasses }}">
            <form action="{{ route('admin.faqs.store') }}" method="POST" class="space-y-4 sm:space-y-5">
                @csrf
                ...
                <div class="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
                    <div class="space-y-4">
                        <div>
                            ...
                            <select id="faq_category_id" name="faq_category_id" required class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}">
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" @selected(old('faq_category_id') == $category->id)>{{ $category->title }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input id="question" type="text" name="question" value="{{ old('question') }}" required class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" placeholder="Enter the FAQ question">
                            @error('question')
                                <p class="mt-1 text-xs font-medium text-rose-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <label for="is_featured" class="flex items-center justify-between gap-3 rounded-2xl border px-3 py-3 text-sm transition {{ $isDark ? 'border-slate-700 bg-slate-900/70' : 'border-slate-200 bg-white' }}">
                                <span>
                                    ...
                                    <span class="block text-xs {{ $mutedTextClasses }}">Highlight this item in the FAQ listing.</span>
                                </span>
                                <input id="is_featured" type="checkbox" name="is_featured" value="1" class="h-4 w-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500" {{ old('is_featured', isset($faq) && $faq->is_featured ? 1 : 0) ? 'checked' : '' }}>
                            </label>
                        </div>
                    </div>
                </div>
