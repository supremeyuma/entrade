<x-layouts.admin>
    ...
    <div class="space-y-3 sm:space-y-6" x-data="previewPhoto()">
            ...
            <div class="relative px-3.5 py-4 sm:px-8 sm:py-8">
                ...
                <div class="relative flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <h1 class="mt-1.5 text-xl font-semibold tracking-tight sm:mt-3 sm:text-4xl {{ $headingClasses }}">Add Trader</h1>
                        <p class="mt-1 text-xs sm:text-sm {{ $mutedTextClasses }}">Create a trader profile with compact inputs and performance metrics that scale well on phones.</p>
                    </div>
        ...
        <section data-aos="fade-up" data-aos-delay="120" class="rounded-[20px] sm:rounded-[28px] border p-3.5 sm:p-6 shadow-sm transition duration-300 ease-out hover:-translate-y-1 hover:shadow-xl {{ $surfaceClasses }}">
            <form action="{{ route('admin.traders.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4 sm:space-y-5">
                @csrf
                ...
                <div class="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                    <div class="space-y-4">
                        <div>
                            ...
                            <input id="name" type="text" name="name" value="{{ old('name') }}" class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" required>
                        </div>

                        <div>
                            <label for="bio" class="mb-1.5 block text-sm font-medium {{ $headingClasses }}">Bio</label>
                            <textarea id="bio" name="bio" rows="5" class="w-full rounded-[22px] border px-3 py-3 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}">{{ old('bio') }}</textarea>
                        </div>
                    </div>
                ...
                <div class="border-t pt-4 {{ $isDark ? 'border-slate-800' : 'border-slate-200' }}">
                    ...
                    <p class="mt-1 text-xs sm:text-sm {{ $mutedTextClasses }}">These values help shape the trader overview cards.</p>
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        <div>
                            <label class="mb-1.5 block text-sm font-medium {{ $headingClasses }}">Total Trades</label>
                            <input type="number" name="performance[total_trades]" step="1" class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" value="{{ old('performance.total_trades') }}">
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium {{ $headingClasses }}">Average ROI (%)</label>
                            <input type="number" name="performance[roi]" step="0.01" class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" value="{{ old('performance.roi') }}">
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium {{ $headingClasses }}">Win Rate (%)</label>
                            <input type="number" name="performance[win_rate]" step="0.01" class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" value="{{ old('performance.win_rate') }}">
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium {{ $headingClasses }}">Max Drawdown (%)</label>
                            <input type="number" name="performance[max_drawdown]" step="0.01" class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" value="{{ old('performance.max_drawdown') }}">
                        </div>
                    </div>
                </div>
