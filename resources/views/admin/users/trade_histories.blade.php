<x-layouts.admin>
    <div class="max-w-7xl mx-auto py-8 px-4">
        <h2 class="mb-4 text-xl font-bold text-gray-800 dark:text-white sm:mb-6 sm:text-2xl">
            Trade History for {{ $user->name }} (User #{{ $user->id }})
        </h2>

        <form method="GET" action="{{ route('admin.users.tradeHistories', $user->id) }}" class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:gap-4">
            <div class="flex-1">
            ...
            <div class="flex-shrink-0">
                <button type="submit" class="inline-flex min-h-[40px] items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                    Filter
                </button>
            </div>
        </form>
            ...
            <table class="min-w-full text-sm text-left">
                <thead class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200">
                    <tr>
                        ...
                        <th class="px-3 py-3 sm:px-4">Trader</th>
                        <th class="px-3 py-3 sm:px-4">
                ...
                <tbody class="text-gray-800 dark:text-gray-100 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($tradeHistories as $history)
                        <tr>
                        <td class="px-3 py-2 sm:px-4">#{{ $history->id }}</td>
                        <td class="px-3 py-2 sm:px-4">{{ $history->trader->name ?? '-' }}</td>
                        <td class="px-3 py-2 sm:px-4">${{ number_format($history->amount_invested, 2) }}</td>
                        <td class="px-3 py-2 sm:px-4">{{ $history->roi }}%</td>
                        <td class="px-3 py-2 sm:px-4">${{ number_format($history->amount_returned, 2) }}</td>
                        <td class="px-3 py-2 sm:px-4">${{ number_format($history->new_trade_balance, 2) }}</td>
                        <td class="px-3 py-2 sm:px-4">{{ $history->created_at ?? '-' }}</td>
