<x-layouts.admin>
    ...
    <div class="space-y-3 sm:space-y-6">
            ...
            <div class="relative px-3.5 py-4 sm:px-8 sm:py-8">
                ...
                <div class="relative flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <h1 class="mt-1.5 text-xl font-semibold tracking-tight sm:mt-3 sm:text-4xl {{ $headingClasses }}">Edit Trade Log</h1>
                        <p class="mt-1 text-xs sm:text-sm {{ $mutedTextClasses }}">Adjust the stored result and notes while keeping the form compact on phones.</p>
                    </div>
        ...
        <section data-aos="fade-up" data-aos-delay="120" class="rounded-[20px] sm:rounded-[28px] border p-3.5 sm:p-6 shadow-sm transition duration-300 ease-out hover:-translate-y-1 hover:shadow-xl {{ $surfaceClasses }}">
            <form action="{{ route('admin.trade-logs.update', $tradeLog) }}" method="POST" class="space-y-4 sm:space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <select name="trader_id" id="trader_id" required class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}">
                            ...
                            @foreach ($traders as $trader)
                                <option value="{{ $trader->id }}" {{ old('trader_id', $tradeLog->trader_id) == $trader->id ? 'selected' : '' }}>{{ $trader->name }}</option>
                            @endforeach
                        </select>
                        @error('trader_id')
                    ...
                    <div>
                        ...
                        <input type="date" name="entry_date" id="entry_date" value="{{ old('entry_date', $tradeLog->entry_date->format('Y-m-d')) }}" required class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}">
                        @error('entry_date')
                            <p class="mt-1 text-xs font-medium text-rose-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="percentage_change" id="percentage_change" value="{{ old('percentage_change', $tradeLog->percentage_change) }}" required class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" placeholder="e.g. 8.50 or -3.00">
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="4" class="w-full rounded-[22px] border px-3 py-3 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" placeholder="Optional notes about this trade log.">{{ old('notes', $tradeLog->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-xs font-medium text-rose-500">{{ $message }}</p>
                    @enderror
                </div>
