<x-layouts.admin>
    ...
    <div class="space-y-3 sm:space-y-6">
            ...
            <div class="relative px-3.5 py-4 sm:px-8 sm:py-8">
                ...
                <div class="relative flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <h1 class="mt-1.5 text-xl font-semibold tracking-tight sm:mt-3 sm:text-4xl {{ $headingClasses }}">Edit FAQ Category</h1>
                        <p class="mt-1 text-xs sm:text-sm {{ $mutedTextClasses }}">Refine naming, icon handling, and description while keeping the form tight on phones.</p>
                    </div>
        ...
        <section data-aos="fade-up" data-aos-delay="120" class="rounded-[20px] sm:rounded-[28px] border p-3.5 sm:p-6 shadow-sm transition duration-300 ease-out hover:-translate-y-1 hover:shadow-xl {{ $surfaceClasses }}">
            <form action="{{ route('admin.faq-categories.update', $faqCategory) }}" method="POST" class="space-y-4 sm:space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input id="title" type="text" name="title" value="{{ old('title', $faqCategory->title) }}" required class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" placeholder="Category title">
                        @error('title')
                    ...
                    <div>
                        ...
                        <input id="slug" type="text" name="slug" value="{{ old('slug', $faqCategory->slug) }}" class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" placeholder="optional-category-slug">
                        @error('slug')
                            <p class="mt-1 text-xs font-medium text-rose-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select id="icon_type" name="icon_type" required class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}">
                            <option value="emoji" @selected(old('icon_type', $faqCategory->icon_type) === 'emoji')>Emoji</option>
                            <option value="image" @selected(old('icon_type', $faqCategory->icon_type) === 'image')>Image</option>
                            <option value="svg" @selected(old('icon_type', $faqCategory->icon_type) === 'svg')>SVG</option>
                            <option value="icon" @selected(old('icon_type', $faqCategory->icon_type) === 'icon')>Font Icon</option>
                        </select>
                        @error('icon_type')
                            <p class="mt-1 text-xs font-medium text-rose-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input id="icon_value" type="text" name="icon_value" value="{{ old('icon_value', $faqCategory->icon_value) }}" required class="w-full rounded-2xl border px-3 py-2.5 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" placeholder="e.g. rocket, /images/icon.svg, fa-solid fa-chart-line">
                        @error('icon_value')
                            <p class="mt-1 text-xs font-medium text-rose-500">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" class="w-full rounded-[22px] border px-3 py-3 text-sm shadow-sm transition focus:outline-none focus:ring-2 {{ $inputClasses }}" placeholder="Optional summary shown alongside this category.">{{ old('description', $faqCategory->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs font-medium text-rose-500">{{ $message }}</p>
                    @enderror
                </div>
